...
import { SplashScreen, LoginScreen, RoleSelection, ResidentRegistration } from './pages/Auth';
import {
...
import {
  SuperAdminDashboard, SuperCondoManager, SuperMarketplaceManager,
  SuperAdminNavigation, SuperPlans, SuperRevenue, SuperSystem
} from './pages/SuperAdmin';

const App: React.FC = () => {
  const [appState, setAppState] = useState<'splash' | 'login' | 'roleSelection' | 'registerResident' | 'main'>('splash');
  ...
  const [serviceRequests, setServiceRequests] = useState<any[]>([
    { id: 1, title: 'Infiltração no Teto', status: 'Aberto', category: 'Manutenção', description: 'Teto da cozinha com manchas de água.', date: '2026-01-05', resident: 'Alex Ferreira', unit: '402-B' },
    { id: 2, title: 'Barulho Excessivo', status: 'Em Análise', category: 'Reclamação', description: 'Vizinho do 502 com som alto após ás 22h.', date: '2026-01-02', resident: 'Clara Mendes', unit: '105-B' }
  ]);
  ...
  try {
    ...
    if (data) {
      setCurrentUser({
        ...
        condoName: data.condominiums?.name || 'Condomínio' // Load name
      });
      setUserRole(data.role as UserRole);
      setAppState('main');
    }
...
const renderContent = () => {
  if (userRole === UserRole.RESIDENT) {
    ...
    switch (activeTab) {
      case 'dashboard': return <SuperAdminDashboard onNavigate={setActiveTab} />;
      case 'super-condos': return <SuperCondoManager onBack={() => setActiveTab('dashboard')} />;
      case 'super-marketplace': return <SuperMarketplaceManager onBack={() => setActiveTab('dashboard')} />;
      case 'super-plans': return <SuperPlans onBack={() => setActiveTab('dashboard')} />;
      case 'super-revenue': return <SuperRevenue onBack={() => setActiveTab('dashboard')} />;
      case 'super-system': return <SuperSystem onBack={() => setActiveTab('dashboard')} />;
      default: return <SuperAdminDashboard onNavigate={setActiveTab} />;
    }
  }
  return null;
};
...
if (appState === 'roleSelection') return <RoleSelection onSelect={handleRoleSelection} />;
  ...
  <div className="relative max-w-md mx-auto shadow-2xl min-h-screen bg-[#f8fafc] overflow-hidden border-x border-slate-100">
    ...
    {!isSubPage && (
      userRole === UserRole.RESIDENT ? (
        ...
        <SuperAdminNavigation activeTab={activeTab} onChange={setActiveTab} />
      ) : null
    )}
  </div>
);
