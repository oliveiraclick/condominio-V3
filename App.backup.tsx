...
const App: React.FC = () => {
  ...
  const [useModernDesign, setUseModernDesign] = useState(() => {
    const saved = localStorage.getItem('beta_modern_design');
    return saved !== null ? saved === 'true' : true; // Default to true now
  });
  ...
  if (appState === 'roleSelection') {
    if (useModernDesign) return <RoleSelectionModern onSelect={(role) => { setUserRole(role === 'resident' ? UserRole.RESIDENT : UserRole.PROFESSIONAL); setAppState(role === 'resident' ? 'registerResident' : 'registerProfessional'); }} onBack={() => setAppState('login')} />;
    return <RoleSelection onSelect={(role) => { setUserRole(role); setAppState(role === UserRole.RESIDENT ? 'registerResident' : 'registerProfessional'); }} onBack={() => setAppState('login')} />;
  }
